...
extension ProfileSetupController: UICollectionViewDelegate, UICollectionViewDataSource {
...
extension ProfileSetupController: SubmitNameDelegate {
    
    func submitName(name: String) {
        if name.isEmpty { return }
        AudioServicesPlaySystemSound(1519)
        self.name = name
        colorButtonCollectionView.reloadData()
        colorButtonCollectionView.selectItem(at: IndexPath(item: 0, section: 0), animated: true, scrollPosition: [])
        userPillPreview.configure(name: name, colorType: selectedColor)
        showOtherInfo()
    }
}
